...
class Tilemap:
    ...
    def tiles_around(self, pos):
        # This will convert the pixel position into a grid position
        # Using this function to only check the 9 tiles around the player at any given time means that no matter the map size, 
        # it will only be looking up 9 tiles, so the program can run quickly, rather than checking all the tiles
        tiles = []
        tile_loc = (int(pos[0] // self.tile_size), int(pos[1] // self.tile_size))
        for offset in NEIGHBOUR_OFFSETS: # Checking x and y offset to see if the tile is in the tilemap
    ...
    def physics_rects_around(self, pos):
        ...
        for tile in self.tiles_around(pos):
            # This will create a Rect that the player can collide with
